...
| Splice | Existing Canton ecosystem infrastructure and technical context | Provides the Canton Network Token Standard context (CIP-0056 and CIP-0112) used by `cap-core` and `cap-recurrence` as the allocation and settlement boundary | External system; Concordia V2 delivers no Splice-specific work |
...
Proposal and repository references are kept in separate columns where applicable. Development Fund proposal links are recorded separately from repository or source links; `N/A` indicates that no such link exists. Proposal state is embedded in the Proposal / PR cell where applicable, while CIPs retain their canonical status in the Repository / source cell as standards rather than Development Fund proposals.
...
| CIP-0082 | N/A | [CIP-0082](https://github.com/canton-foundation/cips/blob/main/cip-0082/cip-0082.md) (Final) | Establishes the 5% Foundation-governed Development Fund and its spend, transparency, and sunset context. It is relevant to the funding and Development Fund governance context for this proposal. | Concordia V2 must remain aligned with the Fund's applicable allocation and transparency expectations; this CIP does not define Concordia runtime behavior. |
