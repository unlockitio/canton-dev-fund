...
| 02. Ongoing Veridica participant lifecycle | [SVG](assets/02-veridica-participant-lifecycle.svg) | [BPMN](assets/02-veridica-participant-lifecycle.bpmn) | The Veridica Platform chassis governs admission, role assignment, suspension, removal, and credential rotation. Candidate participants apply or respond to invitations and supply credentials and conflict disclosures. The chassis consults the Active Governance Mechanism data object (a version-pinned CAP mechanism: ballot, resolution and verification rules) issued and version-controlled by 01B governance, and uses it to admit contributors, aggregators, verifiers or consumers. Each role-party pool instance receives chassis lifecycle deliveries via intermediate message catch events; one boundary message catch handles governance-mandated interruption (suspension, rotation, decommission). |
| 02a. Trust-weight admission and weighting |  |  | Reserved for a future BPMN that models the four trust-weight dimensions (actor type, reputation, history, certifications) and their quota and eligibility effects; the current prose-only description remains in the 2.5 Trust-weight admission and weighting subsection below. |
...
| 03b. Verifier actions | [SVG](assets/03b-veridica-verifier-actions.svg) | [BPMN](assets/03b-veridica-verifier-actions.bpmn) | The Verifier Party (multiplicity 1..999, mutually exclusive with 03a aggregator) starts on Verifier_Authorized. It waits via an event-based gateway for Evidence_Resubmitted (from 03 contributor) or Verification_Request (from 03a aggregator), then either checks evidence and attests with limitations or challenges, or validates method and aggregate-attestation attests with limitations or challenges method or input. |
...
The reusable 01B consortium governance process owns future participant onboarding and lifecycle decisions. Consortium members propose and approve admission, role assignment, suspension, removal, and credential or key rotation. Candidate participants apply or respond to invitations and supply credentials and conflict disclosures. Rejected, expired, challenged, revoked, or insufficient-quorum proposals produce no execution authority. The Veridica Platform pool owns the chassis that executes those governance decisions across the Contributor, Aggregator, Verifier, and Consumer role-party pools. Instead of treating governance as an external lane, the chassis consults the **Active Governance Mechanism** data object: a version-pinned CAP mechanism (ballot, resolution and verification rules) issued and version-controlled by 01B governance. This mechanism is what allows new contributors, aggregators, verifiers or consumers to be admitted, and what governs their subsequent suspension, removal, or credential rotation. Cross-pool delivery from the Veridica Platform is modeled as intermediate message catch events, and one boundary message catch handles governance-mandated interruption (suspension, rotation, decommission). The 03a Aggregator and 03b Verifier are mutually exclusive roles: the same Party cannot be both an aggregator and a verifier for the same product. The downstream 05 reward-allocation workflow is independent of the participant lifecycle: it consumes the versioned participation and reward policy that the participant lifecycle records produce and does not redefine it.
...
The Verifier Party (multiplicity 1..999, mutually exclusive with 03a aggregator) starts on Verifier_Authorized. The verifier waits via an event-based gateway for Evidence_Resubmitted (from 03 contributor) or Verification_Request (from 03a aggregator), then either checks evidence (validation rules, independence, and conflicts) and attests with limitations or challenges, or validates the method (inputs, independence, evidence, and confidence) and aggregate-attestation attests with limitations or aggregate-attestation challenges the method or input. One boundary message catch handles governance suspension mid-verification.
