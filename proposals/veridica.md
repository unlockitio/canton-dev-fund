...
| 03. Data producer actions | [SVG](assets/03-veridica-data-producer-actions.svg) | [BPMN](assets/03-veridica-data-producer-actions.bpmn) | Consolidates Contributor, Aggregator, and independently responsible Verifier actions in distinct lanes. Each authorized producer starts its own role path; no participant lifecycle or Platform actor is modeled. Contributors submit evidence and respond to Verifier requests, challenges, or history-preserving corrections. Aggregators match records under named versioned policy, request eligible inputs, normalize and assess them, request independent review, revise on correction, and publish reviewed aggregates. Verifiers exclusively own substantive contribution and aggregate validation and accountable attestations. A Contributor cannot verify its own contribution where independence is required, and the same party cannot be Aggregator and Verifier for the same product. No message flow reaches 05. |
...
Workflow 03 consolidates the producer actions in distinct Contributor, Aggregator, and Verifier lanes. Each producer-originated path starts when that role is authorized; the participant lifecycle remains outside this workflow. No Veridica Platform actor is modeled because technical transport, persistence, deterministic routing, and catalog infrastructure are not producer behavior.

The Contributor selects the standard version and Observation Key policy and submits a Contribution ID, information or reference, provenance, and evidence. The independently responsible Verifier evaluates the contribution against applicable validation, independence, and conflict rules. The Verifier alone determines whether to request additional evidence, issue a challenge, issue a correction preserving the prior version and reasons, or complete validation with an accountable attestation. The Contributor responds using the submission contract version and returns Evidence_Resubmitted for reevaluation; Verifier_Attestation completes that contribution validation path.

The Aggregator publishes a privacy-safe availability record, applies a named and versioned exact or semantic Matching Policy, manually reviews ambiguous matches or keeps them separate, and invites an eligible independent Verifier. It requests eligible input data and evidence from Contributors, selects authorized input versions, normalizes under a named versioned method, handles outliers, missing data, recency, and minimum independence, and calculates aggregate confidence and traceability. It then requests aggregate verification. The Verifier independently validates the method, inputs, evidence, independence, and confidence and returns Review_Complete with any limitations or Correction_Received for a challenged method or input. A correction returns the Aggregator to input or method revision; only the completed review path publishes the aggregate under access, pricing, and licensing controls.

A Contributor cannot verify its own contribution where independence is required. The same party cannot be both Aggregator and Verifier for the same product. Matching and confidence remain policy-defined and versioned; matching, aggregation, and attestation do not guarantee truth, data quality, agreement, reward, or a governance outcome. The workflow preserves the relevant contribution, input, evidence, correction, attestation, and aggregate-review handoffs. Governance suspension or rotation can interrupt role work without introducing a governance actor lane. No message flow reaches workflow 05 reward allocation.
