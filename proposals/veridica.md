...
| 02. Veridica Access Lifecycle | [SVG](assets/02-veridica-access-lifecycle.svg) | [BPMN](assets/02-veridica-access-lifecycle.bpmn) | Routes sponsored and self-requested admission, additional role or credential requests, governed revocation, and unilateral named role renunciation through the Veridica Operator. Contributor, Aggregator, Verifier, and Consumer are independently offered functional role capabilities, not participant pools; accepted bindings alone create scoped credentials. Governed cases call the reusable 01B mechanism through a collapsed activity. |
...
| 03. Contributor actions | [SVG](assets/03-veridica-contributor-actions.svg) | [BPMN](assets/03-veridica-contributor-actions.bpmn) | The Contributor Party (multiplicity 1..999) starts on Contributor_Authorized issued by the 02 chassis after admission. The contributor selects standard version and Observation Key policy, submits a contribution (Contribution_Submit) to the Verifier, and waits for the Verifier to request evidence, issue a challenge or correction, or complete validation with an accountable attestation. Evidence and correction responses use the contract version of the submission and return as Evidence_Resubmitted for reevaluation. The Verifier exclusively owns receipt of the contribution, substantive evaluation, outcome selection, and attestation actions shown in this workflow; the Contributor receives the Verifier messages and responds as modeled. No Platform actor or infrastructural relay pool is modeled. |
| 03a. Aggregator actions | [SVG](assets/03a-veridica-aggregator-actions.svg) | [BPMN](assets/03a-veridica-aggregator-actions.bpmn) | The Aggregator Party (multiplicity 1..999) starts on Aggregator_Authorized. It publishes the privacy-safe availability record, applies exact or semantic matching, does manual review, invites the 03b verifier (mutually exclusive role), assesses confidence, selects eligible authorized input versions from input Contributors (multiplicity 1..999), normalizes under a named versioned method, handles outliers, calculates aggregate confidence and traceability, excludes or revises, waits for Review_Complete or Correction_Received from the 03b verifier, and publishes the aggregate. No message flow reaches 05. |
| 03b. Verifier actions | [SVG](assets/03b-veridica-verifier-actions.svg) | [BPMN](assets/03b-veridica-verifier-actions.bpmn) | The Verifier Party (multiplicity 1..999, mutually exclusive with 03a aggregator) starts on Verifier_Authorized. It waits via an event-based gateway for Evidence_Resubmitted (routed from 03 contributor) or Verification_Request (from 03a aggregator), owns substantive validation, and may issue Evidence_Request, Challenge, or Correction to the contributor after contribution validation; for aggregate validation it attests with limitations or challenges the method or input. |
| 04. Consumer actions | [SVG](assets/04-veridica-consumer-actions.svg) | [BPMN](assets/04-veridica-consumer-actions.bpmn) | The Consumer Party (single instance) starts on Consumer_Authorized. The consumer discovers the privacy-safe catalog (published by Contributor or Product Publisher, multiplicity 1..999), selects a direct or aggregate product, submits credential, purpose, and access request, then routes through the AC_C_Result and AC_C_Path presentation-layer exclusive gateways. |
...
An approved admission or capability outcome creates one role-specific offer for each proposed capability. The candidate or active participant accepts or declines each capability independently. Only an accepted role capability creates a binding and its scoped credentials; rejected, expired, or declined offers create none. Governed revocation also calls 01B and removes only the named access or capabilities in the approved outcome. By contrast, unilateral self-renunciation verifies participant control and removes the named held capability and its scoped credentials without governance approval, preserving every unrelated capability. The Operator records the request, outcome, offer and response, participant access state, role capability bindings, and lifecycle audit trail.
...
The Contributor Party (multiplicity 1..999) starts on Contributor_Authorized issued by the 02 chassis after admission. The contributor selects the standard version and Observation Key policy and submits a contribution (Contribution_Submit) directly to the Verifier Party. The Verifier receives and evaluates the contribution evidence against the applicable validation, independence, and conflict rules, then determines whether to request additional evidence, issue a challenge, issue a correction preserving the prior version and reasons, or complete validation with an independently accountable attestation. The contributor waits via an event-based gateway for that Verifier action. A single Contributor respond task handles evidence, challenge, and correction responses using the contract version of the submission and sends Evidence_Resubmitted to the Verifier for reevaluation. A successful Verifier_Attestation completes the contribution validation path. No Platform actor is modeled: technical transport, persistence, or deterministic routing is infrastructural and does not require an actor pool in this collaboration. No exclusive gateway branches initial versus correction. One boundary message catch handles governance suspension or rotation arriving mid-submission.

![3. Veridica contributor actions BPMN](assets/03-veridica-contributor-actions.svg)

##### 2.2 Aggregator Actions

The Aggregator Party (multiplicity 1..999) starts on Aggregator_Authorized. The aggregator publishes the privacy-safe availability record, applies exact or semantic matching, does manual review, invites the eligible 03b Verifier (mutually exclusive role), assesses confidence, selects eligible authorized input versions from input Contributors (multiplicity 1..999), normalizes under a named versioned method, handles outliers and missing data, calculates aggregate confidence and traceability, excludes or revises, waits via an event-based gateway for Review_Complete or Correction_Received from the 03b Verifier, and publishes the aggregate under access, pricing, and licensing controls. Message flows that would otherwise call a verifier pool direct exclusively to the 03b Verifier Party pool. No message flow reaches 05 reward allocation or Process_AllocationEffector.

![3a. Veridica aggregator actions BPMN](assets/03a-veridica-aggregator-actions.svg)

##### 2.3 Verifier Actions

The Verifier Party (multiplicity 1..999, mutually exclusive with 03a aggregator) starts on Verifier_Authorized. The verifier waits via an event-based gateway for Evidence_Resubmitted from the 03 contributor workflow or Verification_Request from the 03a aggregator. The Verifier owns substantive contribution validation against validation rules, independence, and conflicts and may issue Evidence_Request, Challenge, or Correction to the contributor after validation, or complete validation with an independently accountable attestation. Technical transport and persistence remain infrastructure rather than actor behavior. For aggregate validation, the Verifier validates the method (inputs, independence, evidence, and confidence) and either aggregate-attestation attests with limitations or challenges the method or input. One boundary message catch handles governance suspension mid-verification.

![3b. Veridica verifier actions BPMN](assets/03b-veridica-verifier-actions.svg)
