...
| 03. Contributor actions | [SVG](assets/03-veridica-contributor-actions.svg) | [BPMN](assets/03-veridica-contributor-actions.bpmn) | The Contributor Party (multiplicity 1..999) starts on Contributor_Authorized issued by the 02 chassis after admission. The contributor selects standard version and Observation Key policy, submits a contribution (Contribution_Submit), waits for the Verifier to issue Evidence_Request, Challenge, or Correction after validation, and responds through one task that handles both initial and correction responses via the contract version of the submission. The Platform only checks the contribution envelope, records messages, routes them deterministically, and forwards them between Contributor and Verifier. |
...
The Contributor Party (multiplicity 1..999) starts on Contributor_Authorized issued by the 02 chassis after admission. The contributor selects the standard version and Observation Key policy, submits a contribution (Contribution_Submit to the Veridica Platform), then waits via an event-based gateway for the Verifier to issue Evidence_Request, Challenge, or Correction after contribution validation. The Platform checks only the contribution envelope, records and deterministically routes the submission to the assigned Verifier, then receives, records, and forwards the Verifier-issued message without governance or substantive assessment. A single Contributor respond task handles both initial and correction responses via the contract version of the submission it receives; the Platform records and deterministically routes that response back to the assigned Verifier. No exclusive gateway branches initial versus correction. One boundary message catch handles governance suspension or rotation arriving mid-submission.
...
The Verifier Party (multiplicity 1..999, mutually exclusive with 03a aggregator) starts on Verifier_Authorized. The verifier waits via an event-based gateway for Evidence_Resubmitted (deterministically routed from 03 contributor by the Platform) or Verification_Request (from 03a aggregator). The Verifier owns substantive contribution validation against validation rules, independence, and conflicts and may issue Evidence_Request, Challenge, or Correction to the contributor after validation; the Platform only records, routes, and forwards those messages. For aggregate validation, the Verifier validates the method (inputs, independence, evidence, and confidence) and either aggregate-attestation attests with limitations or challenges the method or input. One boundary message catch handles governance suspension mid-verification.
