...
| 03. Data producer actions | [SVG](assets/03-veridica-data-producer-actions.svg) | [BPMN](assets/03-veridica-data-producer-actions.bpmn) | Shows contribution versions moving through independent validation, matching, a compact correction and revalidation loop when needed, aggregation into a consolidated contribution, and publication. Verifiers validate and attest individual contribution versions only; they perform no aggregate review or verification. No flow reaches 05. |
...
Workflow 03 shows a compact producer flow in distinct Contributor, Aggregator, and Verifier lanes. Participant lifecycle and technical Platform behavior remain outside this workflow. The Contributor submits a versioned contribution with its Contribution ID, Observation Key, governed standard and policy versions, provenance, evidence, and any relation to an earlier contribution. The independently responsible Verifier validates and attests that individual contribution version against applicable validation, independence, and conflict rules.

The Aggregator matches attested contributions under the named versioned exact or semantic Matching Policy, then evaluates **Correction needed?** A mismatch, ambiguity, stale validation, or other relevant issue causes the Aggregator to request correction. The correction supersedes the affected version and links to it. The prior version and its linked verification remain visible evidence marked **not current / revalidation required**. The corrected contribution is a new version and must receive a new validation or attestation before it can re-enter matching or aggregation.

When no correction is needed, the Aggregator normalizes matched contribution versions under the named versioned Information Standard and Aggregation Policy, applies governed calculation and confidence rules, preserves traceability, and aggregates them into a consolidated contribution. The Aggregator then publishes the consolidated contribution under applicable access, pricing, and licensing controls. The Verifier validates individual contribution versions only and performs zero aggregate review or verification work.
