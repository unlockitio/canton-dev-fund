...
| 03. Data producer actions | [SVG](assets/03-veridica-data-producer-actions.svg) | [BPMN](assets/03-veridica-data-producer-actions.bpmn) | Shows the happy-path choreography across Contributor, Aggregator, and independently responsible Verifier lanes. Contributors submit versioned contribution evidence and provide eligible inputs; Aggregators prepare the round, build a traceable policy-based aggregate, and publish it after independent review; Verifiers own contribution attestation and aggregate review. A Contributor cannot verify its own contribution where independence is required, and the same party cannot be Aggregator and Verifier for the same product. No flow reaches 05. |
...
Workflow 03 shows the happy-path producer choreography in distinct Contributor, Aggregator, and Verifier lanes. Participant lifecycle and technical Platform behavior remain outside this workflow. Each stage hands work to another lane rather than expanding role-internal procedures into separate tasks.

The Contributor submits the standard version, Observation Key policy, Contribution ID, information or reference, provenance, and evidence. The independently responsible Verifier evaluates the contribution against applicable validation, independence, and conflict rules and records an accountable attestation. Evidence requests and challenges remain part of substantive review semantics but are intentionally outside this happy-path view. Corrections and replacements preserve prior versions, evidence, checks, challenges, and reasons.

The Aggregator applies the named versioned exact or semantic Matching Policy, reviews ambiguous matches or keeps them separate, confirms an eligible independent Verifier, and requests authorized inputs. After the Contributor provides eligible input data and evidence, the Aggregator selects the input versions, normalizes them under the named versioned Information Standard and Aggregation Policy, accounts for outliers, missing data, recency, and minimum independence, and calculates policy-based confidence with traceability. The Verifier independently reviews the method, inputs, evidence, independence, and confidence before the Aggregator publishes under access, pricing, and licensing controls.

A Contributor cannot verify its own contribution where independence is required. The same party cannot be both Aggregator and Verifier for the same product. Matching, aggregation, and attestation do not guarantee truth, data quality, agreement, reward, or a governance outcome. No flow reaches workflow 05 reward allocation.
